...
// SỬA Ở ĐÂY 1: Sửa lại hàm search cho đúng API POST mới
...
export default function KeywordAnalysis() {
  ...
  const { data: rankTrackings, isLoading: isLoadingRankings } = useQuery({ queryKey: ['rankTrackings'], queryFn: fetchRankTrackings });

  const generationMutation = useMutation({
    mutationFn: generateKeywordsApi,
    onSuccess: () => {
      toast({ title: "Thành công!", description: "Đã tạo và thêm các từ khóa mới." });
      queryClient.invalidateQueries({ queryKey: ['allKeywords'] });
    },
  ...
  const isFiltering = debouncedFilterTerm.length > 0;
  
  const keywordsToDisplay = isFiltering ? searchedKeywords : allKeywords?.slice(0, visibleCount) || [];
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-gray-500 dark:text-gray-400">
          Khám phá và tạo ra các từ khóa hiệu quả cao cho chiến lược nội dung của bạn.
        </p>
      </div>
        ...
        <TabsContent value="research" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Nhập một từ khóa tiềm năng và AI của chúng tôi sẽ tạo ra một danh sách các từ khóa có liên quan.</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="flex items-center space-x-2">
                ...
                <Button onClick={handleGenerateClick} disabled={generationMutation.isPending}>
                  {generationMutation.isPending ? "Đang khởi tạo..." : <><Rocket className="h-4 w-4 mr-2" /><span>Khởi tạo</span></>}
                </Button>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader><CardTitle>Tìm kiếm từ khóa</CardTitle><CardDescription>Tìm kiếm trực tiếp các từ khóa trong cơ sở dữ liệu của chúng tôi.</CardDescription></CardHeader>
            <CardContent>
              <div className="relative flex-1">
                <Search className="absolute left-3 top-3 h-4 w-4 text-muted-foreground" />
                <Input className="pl-9" placeholder="Nhập bất kỳ một từ khóa nào để chúng tôi kiểm tra ..." value={filterTerm} onChange={(e) => setFilterTerm(e.target.value)} />
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader className="pb-2">
              ...
              <CardDescription>
                {isFiltering
                  ? (isSearching ? `Searching...` : `Found ${searchedData?.pages[0]?.totalItems || 0} results for "${debouncedFilterTerm}"`)
                  : `Showing ${keywordsToDisplay?.length || 0} of ${allKeywords?.length || 0} keywords`
                }
              </CardDescription>
            </CardHeader>
            <CardContent>
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="text-center">Mục đích tìm kiếm</TableHead>
                    <TableHead className="text-center">Xu hướng</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {isLoadingAll && !isFiltering ? (
                    <TableRow><TableCell colSpan={7} className="text-center py-8">Loading initial keywords...</TableCell></TableRow>
                  ) : isSearching && keywordsToDisplay.length === 0 ? (
                    ...
                    <TableRow><TableCell colSpan={7} className="text-center py-8">{isFiltering ? 'No results found.' : 'No keywords to display.'}</TableCell></TableRow>
                  )}
                </TableBody>
              </Table>
              {/* SỬA Ở ĐÂY 4: Logic cho nút "Xem thêm" */}
              {isFiltering ? (
                hasNextSearchPage && (
                ...
                allKeywords && allKeywords.length > visibleCount && (
                  <div className="text-center mt-4">
        ...
        <TabsContent value="tracking" className="space-y-4">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Nhập từ khóa mà bạn muốn theo dõi thứ hạng của từ khóa đó theo thời gian.</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="flex items-center space-x-2">
                <div className="relative flex-1">
          ...
          <Card>
            <CardHeader><CardTitle>Theo dõi thứ hạng từ khóa</CardTitle><CardDescription>Vị trí từ khóa bạn theo dõi.</CardDescription></CardHeader>
            <CardContent>
              <Table>
              ...
              {/* SỬA Ở ĐÂY 5: Logic cho nút "Xem thêm" */}
              {!isFiltering && allKeywords && allKeywords.length > visibleCount && (
                <div className="text-center mt-4">
                  <Button variant="outline" onClick={() => setVisibleCount(prev => prev + 10)}>
                    Xem thêm
                  </Button>
